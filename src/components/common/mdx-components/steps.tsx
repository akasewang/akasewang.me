...
export const Step = ({ title, children }: StepProps) => {
  return (
    <div className="group relative pb-6 pl-8 last:pb-2">
      <div className="absolute bottom-0 left-0 top-0 w-px border-l border-dashed border-border group-last:w-4 group-last:border-b retina:border-l-[0.5px] group-last:retina:border-b-[0.5px]" />
      <div className="absolute -left-3 top-0 z-10 flex size-6 items-center justify-center rounded-lg bg-muted font-mono text-3xs font-medium text-secondary ring-1 ring-border/60 retina:ring-[0.5px] [counter-increment:step] before:content-[counter(step,decimal-leading-zero)]" />
      {title && (
