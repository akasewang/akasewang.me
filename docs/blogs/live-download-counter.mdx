...
```tsx title="src/components/ui/download-counter.tsx"
'use client'
...
```typescript title="src/lib/db/schema.ts"
...
```typescript title="src/lib/actions/views.ts"
'use server'
  ...
  try {
    const [result] = await db
      ...
      .onConflictDoUpdate({
        ...
        set: { installs: sql`${component_metrics.installs} + 1` }, 
      })
...
```tsx title="src/components/live-counter.tsx"
'use client'
...
```typescript title="src/app/r/[slug]/route.ts"
